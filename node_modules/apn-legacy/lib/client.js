  ...
  Client.prototype.write = function write(notification, device, count) {
    ...
    return this.getStream().then(function (stream) {
      ...
      if (_this2.config.token) {
        if (_this2.config.token.isExpired(3300)) {
          _this2.config.token.regenerate(_this2.config.token.generation);
        }
        ...
        stream.on("end", function () {
          if (status === "200") {
            ...
            if (status === "403" && response.reason === "ExpiredProviderToken" && retryCount < 2) {
              ...
              var error = new VError("Error 500, stream ended unexpectedly");
            ...
            var _error = new VError("stream ended unexpectedly");
            resolve({ device: device, error: _error });
          }
        });
